...
import me.rerere.rikkahub.data.ai.mcp.oauth.OAuthAuthorizationLauncher
...
/**
 * MCP OAuth 授权、令牌刷新与持久化（A-lite：内化于 :app，语义与上游一致）。
 *
 * 令牌状态持久化到 [SettingsStore] 的 `mcpServers[].commonOptions.oauth`（app 私有
 * DataStore）。连接生命周期由 McpManager 的配置流消费管理；令牌变化会自然触发连接替换。
 */
internal class McpOAuthCoordinator(
    ...
    suspend fun ensureFreshToken(configInput: McpServerConfig): McpServerConfig {
        ...
        return lock.withLock {
            val config = settingsStore.settingsFlow.first().mcpServers.find { it.id == configInput.id }
                ?: configInput
