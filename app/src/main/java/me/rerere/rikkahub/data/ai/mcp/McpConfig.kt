...
/**
 * OAuth 2.1 授权状态，遵循 MCP 授权规范。
 *
 * 持久化动态客户端注册结果、授权服务器端点以及令牌，用于对需要 OAuth 的 MCP
 * Server 注入 `Authorization: Bearer` 并支持刷新。存于 app 私有 DataStore。
 */
@Serializable
data class McpOAuthState(
    ...
    // 脱敏 toString，避免 client_secret / token 随 config 进入日志。
